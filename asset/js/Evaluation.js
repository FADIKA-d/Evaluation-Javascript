 ...
 function calculateAmount() {
     ...
     // document.getElementById("exo1A").innerHTML = "Le prix unitaire du produit est de " + PU + " euros. "+ "La quantité commandée est de : " + QTECOM + " unité(s)." ;

     //CALCUL
     let TOT = PU * QTECOM; // Calcul du total 
     let REM5 = TOT * 0.05; // Calcul de la remise de 5 %
     let REM10 = TOT * 0.10; // Calcul de la remise de 10 %
     let PORTNOMINAL = (TOT * 0.02 > 6) ? TOT * 0.02 : 6; // Calcul des frais de port
     let PORT = (TOT < 500) ? PORTNOMINAL : 0;
     let REM = 0;
     if (TOT > 200) {
         REM = 10;
         PAP = TOT - REM10 + PORT;
     ...
     let display = `<p>Votre réduction est de : ${REM}</p>
                        <p>Vos frais de port sont de : ${PORT}</p>
                        <p>Le total de votre commande est de : ${PAP}</p>`;
     document.getElementById('displayAmountCommand').innerHTML = display;
...
document.getElementById('calculateCommand').addEventListener('click', calculateAmount);
